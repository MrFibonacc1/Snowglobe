# Task brief for Person C: "Face" (dashboard + demo)
...
2. **Live runs view** (new page "Runs", also replaces Overview's
   ...
   h_agent step output. This is the demo money shot, so make it gorgeous.
...
- With the backend up: build a new workflow in the UI from scratch, trigger
  it with `send_event.py`, watch its run progress step-by-step live in
