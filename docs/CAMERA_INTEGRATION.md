...
`perception/capture.py`, which already speaks RTSP, and go2rtc normalizes
everything to RTSP.
...
rules (the agent dials out); go2rtc handles the vendor-specific protocols so our
code stays protocol-clean.
