...
| **Vaults** | Connect **1Password**; agent gets a `fill_secret_at` action that logs in inside H's infra — password never touches the API or the model. Cloud browsers only. | **Auth mechanism #2.** Store the shop's portal/DoorDash logins; agent logs in fresh each run. | Medium |
...
2. **Browser Profile + Environment for Google Docs.** Sign into Google once, save the profile, make an environment that uses it. *Show:* the agent updating a **private** doc (not just a link-shared one) — proves authenticated action. This is the "it acts as the owner" moment.
